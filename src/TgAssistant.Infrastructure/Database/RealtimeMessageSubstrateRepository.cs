...
public class RealtimeMessageSubstrateRepository : IRealtimeMessageSubstrateRepository
{
    private const string SourceKind = "telegram_realtime_message";
    private const string ProvenanceKind = "telegram_realtime";
    ...
    private readonly ILogger<RealtimeMessageSubstrateRepository> _logger;

    public RealtimeMessageSubstrateRepository(
        IDbContextFactory<TgAssistantDbContext> dbFactory,
        ILogger<RealtimeMessageSubstrateRepository> logger)
    {
        _dbFactory = dbFactory;
        _logger = logger;
    }

    public async Task UpsertRealtimeBatchAsync(IReadOnlyCollection<Message> messages, CancellationToken ct = default)
    {
        var items = messages
            .Where(x => x.Id > 0 && x.ChatId > 0 && x.Source == MessageSource.Realtime)
            .OrderBy(x => x.Id)
            .ToList();
        if (items.Count == 0)
        {
        ...
        foreach (var message in items)
        {
            ...
            source.MetadataJson = JsonSerializer.Serialize(BuildSourceMetadata(message));
        ...
        foreach (var message in items)
        {
            ...
            evidence.ProvenanceJson = JsonSerializer.Serialize(BuildEvidenceProvenance(source, message));
        ...
        _logger.LogInformation(
            "Realtime substrate upserted: messages={MessageCount}, source_objects={SourceCount}, evidence_items={EvidenceCount}",
            items.Count,
    ...
    private static string BuildDisplayLabel(Message message)
        => $"Realtime message {message.TelegramMessageId} in chat {message.ChatId}";
    ...
    private static object BuildSourceMetadata(Message message)
    {
        return new
        {
            ingest_path = "realtime",
    ...
    private static object BuildEvidenceProvenance(DbSourceObject source, Message message)
    {
        return new
        {
            ingest_path = "realtime",
