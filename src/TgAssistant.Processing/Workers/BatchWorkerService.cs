...
public class BatchWorkerService : BackgroundService
{
    ...
    public BatchWorkerService(IMessageQueue queue, IMessageRepository messageRepo, IMediaProcessor mediaProcessor,
        IOptions<BatchWorkerSettings> settings, ILogger<BatchWorkerService> logger)
    {
        _queue = queue; _messageRepo = messageRepo; _mediaProcessor = mediaProcessor;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    if (!_buffers.ContainsKey(msg.ChatId)) _buffers[msg.ChatId] = new ChatBuffer();
            ...
            catch (Exception ex) { _logger.LogError(ex, "Error in batch worker loop"); await Task.Delay(1000, stoppingToken); }
        }
    ...
    private async Task FlushBatchAsync(long chatId, ChatBuffer buffer, CancellationToken ct)
    {
        ...
        foreach (var raw in buffer.Messages)
        {
            ...
            {
                TelegramMessageId = raw.MessageId, ChatId = raw.ChatId,
                SenderId = raw.SenderId, SenderName = raw.SenderName,
                Timestamp = raw.Timestamp, Text = raw.Text,
                MediaType = raw.MediaType, MediaPath = raw.MediaPath,
            ...
            if (mediaLookup.TryGetValue(raw.MessageId, out var mediaResult))
            {
                ...
                { dbMsg.MediaTranscription = mediaResult.Transcription; dbMsg.MediaDescription = mediaResult.Description; }
                else
                { dbMsg.ProcessingStatus = ProcessingStatus.PendingReview; dbMsg.MediaDescription = $"Unrecognized: {mediaResult.FailureReason}"; }
            }
    ...
    private class ChatBuffer
    {
        ...
        {
            if (Messages.Count == 0) FirstMessageAt = DateTime.UtcNow;
